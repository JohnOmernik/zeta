...
INST_DIR="/mapr/$CLUSTERNAME/mesos/$MESOS_ROLE/kafka-rest"
...
echo "Making Directories for kafka-rest"
mkdir -p ${INST_DIR}
cp -R ./conf ${INST_DIR}/
cp install_instance.sh ${INST_DIR}/
chmod +x ${INST_DIR}/install_instance.sh

echo "Kafka-Rest Base Installed"
echo "To install an individual Kafka-Rest instance run:"
echo ""
echo "> /mapr/$CLUSTERNAME/mesos/${MESOS_ROLE}/kafka-rest/install_instance.sh"
echo ""
