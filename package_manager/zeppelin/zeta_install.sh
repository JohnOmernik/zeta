...
APP="zeppelin" # You do want to set this here so change this variable
...
if [ ! -d "/mapr/${CLUSTERNAME}/mesos/${MESOS_ROLE}/dockerimages" ]; then
    echo "The package dockerimages needs to be installed prior to installing $APP"
    echo "exiting"
    rm -rf ${APP_ROOT}
    exit 1
fi

if [ ! -d "/mapr/${CLUSTERNAME}/mesos/${MESOS_ROLE}/drill" ]; then
    echo "The package drill needs to be installed prior to installing $APP"
    echo "exiting"
    rm -rf ${APP_ROOT}
    exit 1
fi
